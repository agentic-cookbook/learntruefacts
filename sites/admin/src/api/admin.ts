import { useQuery, useMutation, useQueryClient } from "@tanstack/react-query";
import type { User, FeatureFlag, MessageLogEntry, FeedbackSubmission } from "learn-true-facts-shared";

function getAuthHeaders(): HeadersInit {
  const raw = localStorage.getItem("admin_auth_tokens");
  if (!raw) return {};
  const { accessToken } = JSON.parse(raw);
  return { Authorization: `Bearer ${accessToken}`, "Content-Type": "application/json" };
}

// ── Users ────────────────────────────────────────────────────────────────

export function useAdminUsers(page = 1, search?: string) {
  return useQuery({
    queryKey: ["admin", "users", page, search],
    queryFn: async () => {
      const params = new URLSearchParams({ page: String(page), limit: "20" });
      if (search) params.set("search", search);

      const res = await fetch(`/api/admin/users?${params}`, { headers: getAuthHeaders() });
      if (!res.ok) throw new Error("Failed to fetch users");
      return res.json() as Promise<{
        users: User[];
        pagination: { page: number; limit: number; total: number };
      }>;
    },
  });
}

export function useUpdateUserRole() {
  const qc = useQueryClient();
  return useMutation({
    mutationFn: async ({ userId, role }: { userId: string; role: string }) => {
      const res = await fetch(`/api/admin/users/${userId}/role`, {
        method: "PATCH",
        headers: getAuthHeaders(),
        body: JSON.stringify({ role }),
      });
      if (!res.ok) throw new Error("Failed to update role");
      return res.json();
    },
    onSuccess: () => qc.invalidateQueries({ queryKey: ["admin", "users"] }),
  });
}

// ── Feature Flags ────────────────────────────────────────────────────────

export function useAdminFlags() {
  return useQuery({
    queryKey: ["admin", "flags"],
    queryFn: async () => {
      const res = await fetch("/api/admin/flags", { headers: getAuthHeaders() });
      if (!res.ok) throw new Error("Failed to fetch flags");
      return res.json() as Promise<{ flags: FeatureFlag[] }>;
    },
  });
}

export function useCreateFlag() {
  const qc = useQueryClient();
  return useMutation({
    mutationFn: async (data: { key: string; description?: string; enabled?: boolean }) => {
      const res = await fetch("/api/admin/flags", {
        method: "POST",
        headers: getAuthHeaders(),
        body: JSON.stringify(data),
      });
      if (!res.ok) throw new Error("Failed to create flag");
      return res.json();
    },
    onSuccess: () => qc.invalidateQueries({ queryKey: ["admin", "flags"] }),
  });
}

export function useToggleFlag() {
  const qc = useQueryClient();
  return useMutation({
    mutationFn: async ({ key, enabled }: { key: string; enabled: boolean }) => {
      const res = await fetch(`/api/admin/flags/${key}`, {
        method: "PATCH",
        headers: getAuthHeaders(),
        body: JSON.stringify({ enabled }),
      });
      if (!res.ok) throw new Error("Failed to toggle flag");
      return res.json();
    },
    onSuccess: () => qc.invalidateQueries({ queryKey: ["admin", "flags"] }),
  });
}

export function useDeleteFlag() {
  const qc = useQueryClient();
  return useMutation({
    mutationFn: async (key: string) => {
      const res = await fetch(`/api/admin/flags/${key}`, {
        method: "DELETE",
        headers: getAuthHeaders(),
      });
      if (!res.ok) throw new Error("Failed to delete flag");
      return res.json();
    },
    onSuccess: () => qc.invalidateQueries({ queryKey: ["admin", "flags"] }),
  });
}

// ── Messaging ────────────────────────────────────────────────────────────

export function useMessageLog(page = 1) {
  return useQuery({
    queryKey: ["admin", "messaging", "log", page],
    queryFn: async () => {
      const res = await fetch(`/api/admin/messaging/log?page=${page}`, { headers: getAuthHeaders() });
      if (!res.ok) throw new Error("Failed to fetch message log");
      return res.json() as Promise<{
        messages: MessageLogEntry[];
        pagination: { page: number; limit: number; total: number };
      }>;
    },
  });
}

export function useSendMessage() {
  const qc = useQueryClient();
  return useMutation({
    mutationFn: async (data: { channel: "email" | "sms"; to: string; subject?: string; body: string }) => {
      const res = await fetch("/api/admin/messaging/send", {
        method: "POST",
        headers: getAuthHeaders(),
        body: JSON.stringify(data),
      });
      if (!res.ok) throw new Error("Failed to send message");
      return res.json();
    },
    onSuccess: () => qc.invalidateQueries({ queryKey: ["admin", "messaging"] }),
  });
}

// ── Feedback ─────────────────────────────────────────────────────────────

export function useFeedback(page = 1, status?: string) {
  return useQuery({
    queryKey: ["admin", "feedback", page, status],
    queryFn: async () => {
      const params = new URLSearchParams({ page: String(page) });
      if (status) params.set("status", status);

      const res = await fetch(`/api/admin/feedback?${params}`, { headers: getAuthHeaders() });
      if (!res.ok) throw new Error("Failed to fetch feedback");
      return res.json() as Promise<{
        feedback: FeedbackSubmission[];
        pagination: { page: number; limit: number; total: number };
      }>;
    },
  });
}

export function useUpdateFeedback() {
  const qc = useQueryClient();
  return useMutation({
    mutationFn: async ({ id, ...data }: { id: string; status?: string; adminNotes?: string }) => {
      const res = await fetch(`/api/admin/feedback/${id}`, {
        method: "PATCH",
        headers: getAuthHeaders(),
        body: JSON.stringify(data),
      });
      if (!res.ok) throw new Error("Failed to update feedback");
      return res.json();
    },
    onSuccess: () => qc.invalidateQueries({ queryKey: ["admin", "feedback"] }),
  });
}
